...
double MidpointIntegral::Calc(const double& value) const { return _irange.Length() * _func.Calc((_irange.getFloor() + _irange.getCailing()) / 2); }
...
double TrapeziumIntegral::Calc(const double& value) const { return (_func.Calc(_irange.getFloor()) + _func.Calc(_irange.getCailing())) * _irange.Length() / 2; }
