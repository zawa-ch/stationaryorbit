...
StationaryOrbit::Graphics::BitmapByteBuffer::BitmapByteBuffer(const ImageInfomation& info)
	: BitmapByteBuffer(
		info.getSize(),
...
StationaryOrbit::Graphics::BitmapByteBuffer& StationaryOrbit::Graphics::BitmapByteBuffer::operator=(const BitmapByteBuffer& value)
{
	...
	// 内容コピー
	std::copy<uint8_t*, uint8_t*>(_data, _data + CalcLength(value._size, value._ch), _data);
